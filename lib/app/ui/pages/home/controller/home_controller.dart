...
class HomeController extends SimpleNotifier {
  ...
  ExpenseDirection _expenseDirection = ExpenseDirection.received;

  ExpenseDirection get expenseDirection => _expenseDirection;
  ...
  void setCurrentExpenseDirection(ExpenseDirection? expenseDirection) {
    _expenseDirection = expenseDirection ?? this.expenseDirection;
    notify();
  }

  void setCurrentExpenseType(ExpenseType? expenseType) {
    _expenseType = expenseType ?? this.expenseType;
    notify();
  }
  ...
  double amount() {
    ...
    for (var element in _listDismissibles) {
      total = element.expense.direction == ExpenseDirection.sent
          ? total - element.expense.price
          : total + element.expense.price;
    }
    _totalAmount = total;
    notify();
  ...
  Future<void> uploadExpenseToFirebaseFirestore() async {
    await _firebase.uploadExpenseToFirebaseFirestore(
      ...
      expenseDirection: _expenseDirection,
        ...
        _listDismissibles = event.docs.map(
          (e) {
            ...
            return MyDismissible(
              expense: Expense(
                direction: EnumToString.fromString(
                      ExpenseDirection.values,
                      e.get('direction'),
                    ) ??
                    ExpenseDirection.received,
  ...
  void openCategoryExpenses(ExpenseType type) {
    router.pushNamed(
      Routes.CATEGORY,
      arguments: {
        'allExpenses': _listDismissibles,
